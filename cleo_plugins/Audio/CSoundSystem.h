...
namespace CLEO
{
    ...
    inline bool isNetworkSource(const char* path)
    {
    ...
    inline float lerp(float a, float b, float progress)
    {
        return a * (1.0f - progress) + b * progress;
    }
    inline CVector lerp(CVector a, CVector b, float progress)
    {
        return a * (1.0f - progress) + b * progress;
    }
    inline BASS_3DVECTOR toBass(const CVector& v)
    {
